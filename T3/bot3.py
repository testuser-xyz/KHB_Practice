...
#system prompts for the voice assistant (customer perspective)
...
async def bot(runner_args: RunnerArguments):
    ...
    async def handle_client_connected(transport: SmallWebRTCTransport, client):
        ...
        print("="*80)
        print("🤖 Acting as customer - waiting for restaurant to speak first...")
        ...
        # Customer waits for restaurant to speak first
        # If restaurant doesn't speak within 5 seconds, customer initiates
        greeting_prompt = get_greeting_prompt(time_context)
        messages.append({"role": "system", "content": greeting_prompt})
        
        # Schedule a timeout: if user doesn't speak in 5 seconds, bot speaks first
        async def greeting_timeout():
            await asyncio.sleep(5)
            # Only speak if no user input has been received yet
            if len([m for m in messages if m["role"] == "user"]) == 0:
                print("⏱️ 5 seconds passed - Customer speaking first...")
                customer_prompt = "The restaurant hasn't greeted you yet. Say hello and ask to place an order. Keep it brief and natural."
                messages.append({"role": "system", "content": customer_prompt})
                await task.queue_frames([LLMRunFrame()])
        
        asyncio.create_task(greeting_timeout())
